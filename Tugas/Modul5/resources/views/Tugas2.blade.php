...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('assets/css/Tugas2.css') }}">
...
<body>
    <div class="container">
        ...
        <section id="movies">
            ...
            <div class="cd-grid" id="cdGrid">
                <!-- Loop through cinemas and display them -->
                @foreach ($data as $row)
                <div class="cd-card">
                    <img src="{{ Storage::url('cinemas' . $row->cinema_image) }}" alt="{{ $row->cinema_name }}">
                    <div class="cd-info">
                        <h4 class="cd-title">{{ $row->cinema_name }}</h4>
                        <div class="price">${{ number_format($row->price, 2) }}</div>
                        <button class="add-to-cart" 
                                data-id="{{ $row->id }}" 
                                data-title="{{ $row->cinema_name }}" 
    ...
    <script>
        ...
        var cinemaButtons = document.getElementsByClassName("add-to-cart");
        for (var i = 0; i < cinemaButtons.length; i++) {
            cinemaButtons[i].addEventListener("click", function (event) {
                ...
                xhr.open("GET", "/connection?id=" + id, true);
                xhr.send();
            });
        }
    </script>
